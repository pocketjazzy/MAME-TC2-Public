...
enum : unsigned {
	...
	LINKPLAY_CONFIG,        // P068 (CrouchCrisis): namco_c139 link settings, timecrs2 family only
...
void menu_main::populate()
{
	...
	// P068 (CrouchCrisis tc2-linkplay): "Link Play Config" sits between DIP
	// Switches and Machine Configuration, and ONLY when the running machine
	// actually carries a namco_c139 link device - all other machines' menus
	// are untouched.  MODEL PROVENANCE: Fable 5.
	if (menu_linkplay::find_c139(machine()))
		item_append(_("menu-main", "Link Play Config"), 0, (void *)LINKPLAY_CONFIG);
...
bool menu_main::handle(event const *ev)
{
	...
	if (ev && (ev->iptkey == IPT_UI_SELECT))
	{
		...
		case LINKPLAY_CONFIG:
			menu::stack_push<menu_linkplay>(ui(), container());
			break;
